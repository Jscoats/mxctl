"""Tests for the `my mail count` command."""
...
def test_count_all_accounts(monkeypatch, mock_args, capsys):
    ...
    from my_cli.commands.mail.accounts import cmd_count

    mock_run = Mock(return_value="7")
    monkeypatch.setattr("my_cli.commands.mail.accounts.run", mock_run)
    monkeypatch.setattr("my_cli.commands.mail.accounts.resolve_account", lambda _: None)
...
def test_count_specific_account(monkeypatch, mock_args, capsys):
    ...
    monkeypatch.setattr("my_cli.commands.mail.accounts.run", mock_run)
    monkeypatch.setattr("my_cli.commands.mail.accounts.resolve_account", lambda _: "iCloud")
